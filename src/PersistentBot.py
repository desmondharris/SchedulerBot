...
class PersistentBot:
    ...
    async def web_app_data(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        match split_data[0]:
            case "NONRECURRINGEVENT":
                self.create_nr_event(context.job.data["chat_id"], split_data[1], datetime.datetime.strptime(' '.join([split_data[2], split_data[3]]), "%Y-%m-%d %H:%M"))
    ...
    def create_nr_event(self, chat_id, name: str, event_time: datetime.datetime):
        ...
        # self.db_insert("events", {"name": name, "time": event_time, "user": chat_id})
    ...
    def db_insert(self, table, data: dict):
        ...
        if table == "users":
            ...
            values = (data['name'], data['datetime'], data['user'])
            cursor.execute(query, values)
            self.conn.commit()
            cursor.close()
...
b = PersistentBot()
b.db_insert("users", {"chatid": 1710495555})
b.start_bot()
